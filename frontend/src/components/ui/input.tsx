...
const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<'input'> & { isGroup?: boolean }>(
  ({ className, type, isGroup = false, ...props }, ref) => {
    return (
      <input
        type={type}
        className={cn(
          ...
          isGroup ? '' : 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
          className
        )}
        ref={ref}
        {...props}
